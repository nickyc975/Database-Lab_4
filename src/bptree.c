...
node_t *_new_node(addr_t addr);
node_t *_new_inner(addr_t addr);
node_t *_new_leaf(addr_t addr);

int _save_node(node_t *node, int free_after_save);

value_blk_t *_new_value_blk(addr_t addr);
int _save_value_blk(value_blk_t *value_blk, int free_after_save);
...
void bptree_init(bptree_t *bptree,bptree_meta_t *meta)
{
    ...
    if (bptree->root_node)
    {
        ...
        {
            bptree->leftmost_node = read_node(meta->leftmost_addr);
    ...
    {
        bptree->root_node = _new_leaf(meta->root_addr);
        bptree->leftmost_node = bptree->root_node;
        _save_node(bptree->root_node, 0);
    }
    else
    {
...
{
    _print_node(bptree->root_node->addr);

    addr_t next_addr;
    node_t *node = read_node(bptree->leftmost_node->addr);
...
void bptree_free(bptree_t *bptree)
{
    if (bptree->root_node)
    {
        free_node(bptree->root_node);
    }

    if (bptree->leftmost_node)
    {
        free_node(bptree->leftmost_node);
    }
}
...
key_iter_t *new_key_iter(bptree_meta_t *meta)
{
    if (meta->leftmost_addr)
    {
        ...
        iter->curr_node = read_node(meta->leftmost_addr);
        if (!iter->curr_node)
        {
...
inline int next_key(key_iter_t *iter)
{
    ...
    else if (iter->curr_node->next_node != 0)
    {
        ...
        free_node(iter->curr_node);
        iter->curr_node = read_node(next_addr);
        if (!iter->curr_node)
        {
            printf("Invalid node addr %d\n", next_addr);
            exit(1);
...
inline node_t *read_node(addr_t addr)
{
    return (node_t *)readBlockFromDisk(addr, buffer);
}
...
{
    freeBlockInBuffer((unsigned char *)node, buffer);
}

inline value_blk_t *read_value_blk(addr_t blk_addr)
{
    return (value_blk_t *)readBlockFromDisk(blk_addr, buffer);
}
...
value_blk_iter_t *new_value_blk_iter(addr_t addr)
{
    if (addr)
    {
        ...
        iter->init_addr = addr;
        iter->curr_blk = read_value_blk(addr);
        if (!iter->curr_blk)
        {
            printf("Invalid node addr %d\n", addr);
            exit(1);
...
inline addr_t next_value(value_blk_iter_t *iter)
{
    ...
    else if (iter->curr_blk->next_blk_addr != 0)
    {
        ...
        free_value_blk(iter->curr_blk);
        iter->curr_blk = read_value_blk(next_addr);
        if (!iter->curr_blk)
        {
...
inline void free_value_blk(value_blk_t *value_blk)
{
    freeBlockInBuffer((unsigned char *)value_blk, buffer);
}
...
inline node_t *_new_node(addr_t addr)
{
    node_t *node = (node_t *)getNewBlockInBuffer(buffer);
    node->key_num = 0;
    node->addr = addr;
    return node;
}

inline node_t *_new_inner(addr_t addr)
{
    node_t *node = _new_node(addr);
    node->type = INNER;
    return node;
}

inline node_t *_new_leaf(addr_t addr)
{
    node_t *node = _new_node(addr);
    node->type = LEAF;
    node->next_node = 0;
...
inline int _save_node(node_t *node, int free_after_save)
{
    int result = writeBlockToDisk((unsigned char *)node, node->addr, buffer);
    if (free_after_save)
    {
        free_node(node);
    }
...
inline value_blk_t *_new_value_blk(addr_t addr)
{
    value_blk_t *value_blk = (value_blk_t *)getNewBlockInBuffer(buffer);
    value_blk->value_num = 0;
    value_blk->blk_addr = addr;
    value_blk->next_blk_addr = 0;
    return value_blk;
}

inline int _save_value_blk(value_blk_t *value_blk, int free_after_save)
{
    int result = writeBlockToDisk((unsigned char *)value_blk, value_blk->blk_addr, buffer);
    if (free_after_save)
    {
...
{
    addr_t next_addr;
    node_t *node = read_node(bptree->root_node->addr);
...
void _insert_into_leaf(bptree_t *bptree, node_t *node, int key, addr_t value)
{
    ...
    if (node && node->type == LEAF)
    {
        for (int i = 0; i < node->key_num; i++)
        {
            if (node->keys[i] == temp_key)
            {
                value_blk = read_value_blk(node->blk_addrs[i]);
                value_blk->values[value_blk->value_num] = value;
                value_blk->value_num++;
                if (value_blk->value_num >= MAX_VALUE_NUM)
                {
                    _save_value_blk(value_blk, 1);
                    value_blk = _new_value_blk(_next_addr(bptree));
...
void _split_inner(bptree_t *bptree, stack_t *addr_stk)
{
    ...
    node_t *node = read_node(stk_pop(addr_stk));
    unsigned int spliter_idx = (node->key_num + 1) / 2;

    new_node = _new_inner(_next_addr(bptree));
    ...
    {
        parent = read_node(stk_top(addr_stk));
...
void _split_leaf(bptree_t *bptree, node_t *node, stack_t *addr_stk)
{
    ...
    new_node = _new_leaf(_next_addr(bptree));
